...
def calculate_workload(data):
    try:
        ...
        H_base = L * (1 + D_percent) * NSC / NP  # Base preparation time
        ...
        # Preparation Time
        HP = (H_base * L_Y * (1 + CC) + 
             (PT_f * (1 - RT_ta) * T + PP_f * (1 - RP_ta) * P)) / 60
        ...
        detailed_calculations = {
            ...
            "Preparation Time": {
                "NATURAL_LANGUAGE": [
                    "Based on your input:",
                    f"You reported spending {H_base:.2f} hours on base preparation, with {CC*100:.2f}% of content changes compared to previous offerings.",
                    "",
